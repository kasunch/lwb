...
static inline void update_sched_stats(void) {
	if (IS_SYNCED()) {
		if (first_schedule_rcvd) {
			uint8_t diff = sched.seq_no - last_rcvd_seq_no;
			sched_tot += diff;
			sched_rcvd++;
			while (sched_tot >= SCHED_THR) {
				sched_tot = (sched_tot % 2) + (sched_tot / 2);
				sched_rcvd = (sched_rcvd % 2) + (sched_rcvd / 2);
			}
			ps[node_id] = (sched_tot) ? ((uint16_t)((10000LLU * sched_rcvd)  / sched_tot)) : 0;
		} else {
			first_schedule_rcvd = 1;
		}
		last_rcvd_seq_no = sched.seq_no;
	}
	Ts[node_id] = (Ts_cnt) ? ((uint16_t)((1000000LLU * Ts_tot) / Ts_cnt / RTIMER_SECOND)) : 0;
	Td[node_id] = (Td_cnt) ? ((uint16_t)((1000000LLU * Td_tot) / Td_cnt / RTIMER_SECOND)) : 0;
}
	...
	if (sync_state == SYNCED) {
		if (joining_state == NOT_JOINED) {
			if (n_stream_reqs == 0) {
				if (is_sink) {
					n_stream_reqs = 1;
					stream_reqs[0].req_type = DATA_SINK & 0x3;
				} else {
					if (is_source) {
						n_stream_reqs = 1;
...
//			if (node_id == 2 && random_rand() > 36045) {
//				SET_UNSYNCED();
//			}
			if (random_rand() > 58982) {
				SET_UNSYNCED();
			}
			if (sync_state != BOOTSTRAP) {
				update_Ts();
			...
			if ((sync_state == SYNCED || sync_state == UNSYNCED_1) && GET_COMM(PERIOD)) {
				// communication
				uncompress_schedule(get_data_len());
