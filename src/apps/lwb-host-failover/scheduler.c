...
static rtimer_clock_t t_sched_start, t_sched_stop;
...
void process_stream_req(uint16_t id, stream_req req) {
	...
	if (GET_STREAM_ID(req.req_type) == 1 &&
			...
			id ==15 || id == 29 || id == 33)) {
#else
	if (GET_STREAM_ID(req.req_type) == 1 && id != node_id) {
#endif /* FAIRNESS */
...
void scheduler_init(uint16_t curr_time) {
	// initialize streams member and list
	memb_init(&streams_memb);
	list_init(streams_list);

	// initialize persistent variables
	scheduler_init_time = curr_time;
	last_free_reqs_time = curr_time;
	time = curr_time + INIT_PERIOD * TIME_SCALE;
	n_slots_to_assign = 0;
	period = INIT_PERIOD;
	PERIOD = period | SET_COMM(1);
	if (period != 1) {
