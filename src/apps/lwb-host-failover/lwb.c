...
static uint16_t bootstrap_seconds;
static uint16_t scheduler_init_time;
static uint16_t channels[] = CHANNELS;
static uint16_t hosts[] = HOSTS;
static uint16_t channel_idx = 0;
static uint16_t t_last_tx;
static uint16_t host_timeout = HOST_TIMEOUT;
#define N_CHANNELS (sizeof(channels) / 2)

static uint16_t t_remove_h1[] = T_REMOVE_H1;
static uint16_t t_add_h1[] = T_ADD_H1;
static uint16_t t_remove_h2[] = T_REMOVE_H2;
static uint16_t t_add_h2[] = T_ADD_H2;
static uint16_t t_remove_h3[] = T_REMOVE_H3;
static uint16_t t_add_h3[] = T_ADD_H3;
...
static inline void set_host(void) {
	if (node_id == hosts[channel_idx]) {
		is_host = 1;
	} else {
		is_host = 0;
	}
	cc2420_set_channel(channels[channel_idx]);
}
...
void set_hosts_sinks_sources(void) {
	...
	if (sizeof(sources) == 0) {
		// all nodes but the hosts and the sinks are sources
		if (!is_host && !is_sink) {
			is_source = 1;
		}
	} else {
...
static struct pt pt_sync, pt_rr, pt_rr_host;
...
PROCESS_THREAD(lwb_init, ev, data)
{
	...
	if (is_host) {
		...
		if (is_source) {
			...
			stream_reqs[0].time_info = INIT_DELAY * TIME_SCALE;
			t_last_req = 0;
		}
#else
		n_stream_reqs = 0;
...
interrupt(PORT2_VECTOR) irq_p2(void) {
	...
	if (is_source) {
		n_stream_reqs = 1;
		if (stream_reqs[0].ipi == HIGH_IPI) {
			stream_reqs[0].ipi = LOW_IPI;
		} else {
			stream_reqs[0].ipi = HIGH_IPI;
		}
